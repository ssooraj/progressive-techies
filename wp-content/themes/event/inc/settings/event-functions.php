...
,'post__in'  => get_option( 'sticky_posts' )));
	if($query->have_posts() && get_option( 'sticky_posts' )){
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'event_slider_image');
				...
				if (!empty($banner) && isset($banner['url']) && !empty($banner['url'])) {
					$event_sticky_post_sliders_display 	.= '<div class="image-slider" title="'.the_title('', '', false).'"' .' style="background-image:url(' ."'" .esc_url($banner['url'])."'" .')">';
...
function event_scripts() {
	...
	wp_enqueue_script('bootstrap-js', get_template_directory_uri().'/js/bootstrap.3.3.7.min.js', array('jquery'));

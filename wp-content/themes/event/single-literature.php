...
<div id="primary">
	...
	if( have_posts() ) {
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class();?>>
				<header class="entry-header">
					...
					<div class="entry-meta">
						...
						<span class="author vcard"><a href="#" title="<?php  echo $writer.', '.$company; ?>">
						<?php echo $writer.', '.$company; ?> </a></span> <span class="posted-on"><a title="<?php echo esc_attr( get_the_time() ); ?>" href="<?php the_permalink(); ?>">
		...
		<?php $disable_entry_format = $event_settings['event_entry_format_blog'];
			if($disable_entry_format =='show' || $disable_entry_format =='show-button' || $disable_entry_format =='hide-button'){ ?>
				<footer class="entry-footer">
					<div class="entry-meta">
						...
						<?php 
						$malayalam = the_terms( $post->ID, 'malayalam');
						$english = the_terms( $post->ID, 'english');
						if($english || $malayalam) {
							$taxonomies = 'Categories: ';
							if($english) {
								$taxonomies .= $english;
							} else {
								$taxonomies .= $malayalam;
							}
							echo $taxonomies;
						}?>
						</span> <!-- end .cat-links -->
			...
			<?php }
			echo do_shortcode('[wpdevart_facebook_comment curent_url="'.get_the_permalink().'" order_type="reverse_time" title_text="" title_text_color="#000000" title_text_font_size="22" title_text_font_famely="monospace" title_text_position="left" width="100%" bg_color="#d4d4d4" animation_effect="random" count_of_comments="20" ]'); 
				if( is_attachment() ) { ?>

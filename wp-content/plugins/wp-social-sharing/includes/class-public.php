...
class SS_Public {
	...
	public function add_links_after_content( $content ){
		...
		$show_status = get_post_meta( get_the_ID(), '_disabel_social_sharing', true );

		if( ! empty( $opts['auto_add_post_types'] ) && 
			in_array( get_post_type(), $opts['auto_add_post_types'] ) && 
			is_singular( $opts['auto_add_post_types'] ) &&
			$show_status != 'disable' ) {
			$show_buttons = true;
		}
	        ...
	        foreach($icon_order as $o) {
	        	switch($o) {
	        		case 'f':
	        			...
	        			?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo '';?>" href="http://www.facebook.com/sharer/sharer.php?u=<?php echo $url; ?>" target="_blank" ><i class="fa fa-facebook-square" aria-hidden="true"></i></a><?php
	        			}
	        		break;
	        		case 't':
	        			if(in_array('twitter', $social_options)){
	        			?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo '';?>" href="http://twitter.com/intent/tweet/?text=<?php echo $title; ?>&url=<?php echo $url; ?><?php if(!empty($twitter_username)) {  echo '&via=' . $twitter_username; } ?>" target="_blank"><i class="fa fa-twitter-square" aria-hidden="true"></i></a><?php
	        			}
	        		break;
	        		case 'g':
	        			if(in_array('googleplus', $social_options)){
	        			?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo '';?>" href="https://plus.google.com/share?url=<?php echo $url; ?>" target="_blank" ><i class="fa fa-google-plus-square" aria-hidden="true"></i></a><?php
	        			}
	        		break;
					case 'l':
